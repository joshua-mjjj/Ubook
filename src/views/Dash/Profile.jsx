...
function BlockBookings(props) {
  ...
  React.useEffect(() => {
    if(props.bookings !== null){
      const list_bookings = props.bookings.filter((booking) => booking.cancelled === false && booking.block_booking === true);
  ...
  const format_date = (date) => {
    ...
    const new_date_ = new_date.toString().substring(0, 16);
    return new_date_;
  };
  ...
  return (
    ...
    <div style={{ marginTop : "10px"}}>
      <p className={classes.sub_title} >Profile</p>
    </div>
                    ...
                    <TableBody>
                      {data.map((row, index) => (
                        <StyledTableRow key={row.name}>
                          ...
                          <StyledTableCell align="left">{format_date(row.start_date)}</StyledTableCell>
                          <StyledTableCell align="left">{format_date(row.end_date)}</StyledTableCell>
